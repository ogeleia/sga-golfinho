<%-- 
    Document   : sgaEditaCadastroTurma
    Created on : 10/12/2007, 14:49:53
    Author     : Hwoarang
--%>
...
function ValidaFormulario(){
 ...
 if ( document.editarturma.descricao.value == "") {
  erro = erro + "Preencha a Descrição \n"
 }
 if ( document.editarturma.modalidade.value == "") {
  erro = erro + "Preencha o Modalidade \n"
 }
 if ( document.editarturma.classe.value == "") {
  erro = erro + "Preencha a Classe \n"
 }
 if ( document.editarturma.horario.value == "") {
  erro = erro + "Preencha o Horário \n"
 }
 if ( document.editarturma.codigoprofessor.value == "") {
  erro = erro + "Preencha o código do professor \n"
 }

if(erro == ""){
  document.editarturma.submit();
 }else{
 	alert(erro);
 }
}
...
<form name="editarturma" action="editarturma.do" method="post">
                 ...
                 <tr>
                  ...
                  <tr>
                    ...
                    <td  valign="top" bgcolor="a0d7f3" class="CelulaCentro"><table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#FFFFFF">
                      ...
                      <input type="hidden" name="cod" value="${turma.codigo}">
                      <tr>
                        <td width="11%" class="style1">Descrição:</td>
                        <td colspan="3" valign="middle"><input name="descricao" type="text" id="descricao" size="50" maxlength="100" value="${turma.descricao}">
                      ...
                      <tr>
                        ...
                        <td colspan="3"><select name="modalidade">
                        	<option value="">-</option>
                            <c:forEach var="modalidade" items="${modalidadeList}">
                             <option <c:if test="${modalidade.codigo == turma.modalidade}"> selected </c:if> value="<c:out value="${modalidade.codigo}"/>"><c:out value="${modalidade.descricao}"/></option>                            </c:forEach>
                        </select></td>
                      </tr>
                      <tr>
                        ...
                        <td colspan="3"><select name="classe">
                            ...
                            <c:forEach var="classe" items="${classeList}">
                             <option <c:if test="${classe.codigo == turma.classe}"> selected </c:if> value="<c:out value="${classe.codigo}"/>"><c:out value="${classe.classe}"/></option>                            </c:forEach>
                        </select></td>
                      </tr>
                      <tr>
                        <td class="style1">Horário:</td>
                        <td colspan="3"><input name="horario" type="text" id="horario" size="25" maxlength="45" value="${turma.horario}"></td>
                      </tr>
                      <tr>
                        ...
                        <td colspan="3"><select name="codigoprofessor">
                            ...
                            <c:forEach var="professor" items="${professorList}">
                             <option <c:if test="${professor.codigo == turma.codigoprofessor}"> selected </c:if> value="<c:out value="${professor.codigo}"/>"><c:out value="${professor.nome}"/></option>                            </c:forEach>
                        </select></td>
                      </tr>
                      <tr>
                        ...
                        <td colspan="3"><label>
                          <select name="status">
                            <option value="1" <c:if test="${turma.status == '1'}"> selected </c:if> >Ativado</option>
                            <option value="0" <c:if test="${turma.status == '0'}"> selected </c:if> >Desativado</option>
                          </select>
                        </label></td>
                      </tr>
                      </c:forEach>
